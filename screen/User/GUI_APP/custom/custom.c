...
void custom_init(lv_ui *ui)
{
    ...
    lv_obj_set_style_bg_img_src(guider_ui.screen_slider_1, &_icn_slider_alpha_25x25, LV_PART_KNOB);
    spectrum = spectrum_1;
...
static lv_obj_t * album_img_create(lv_obj_t * parent)
{
    ...
    switch(track_id) {
    case 2:
        lv_img_set_src(img, &_cover_3_alpha_175x175);
        spectrum = spectrum_3;
        spectrum_len = sizeof(spectrum_3) / sizeof(spectrum_3[0]);
        break;
    case 1:
        lv_img_set_src(img, &_cover_2_alpha_175x175);
        spectrum = spectrum_2;
        spectrum_len = sizeof(spectrum_2) / sizeof(spectrum_2[0]);
        break;
    case 0:
        lv_img_set_src(img, &_cover_1_alpha_175x175);
        spectrum = spectrum_1;
        spectrum_len = sizeof(spectrum_1) / sizeof(spectrum_1[0]);
        break;
    }
...
void lv_demo_music_list_btn_check(uint32_t track_id, bool state)
{
    ...
    if(state) {
        ...
        lv_img_set_src(icon, &_btn_list_pause_alpha_70x70);
        lv_obj_scroll_to_view(btn, LV_ANIM_ON);
    }
    else {
        lv_obj_clear_state(btn, LV_STATE_PRESSED);
        lv_img_set_src(icon, &_btn_list_play_alpha_70x70);
    }
}
...
static void spectrum_draw_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        static int32_t center_value = 175;
