...
u32 LCD_ReadPoint(u16 x,u16 y)
{
	...
	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x7789)LCD_WR_REG(0X2E);//9341/3510/1963 发送读GRAM指令
...
{ 	  
	...
	GPIO_Initure.Pin=GPIO_PIN_10;            //PB10, rst
	HAL_GPIO_Init(GPIOB,&GPIO_Initure); 
    /*
	...
	*/
	...
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FMC_Bank1E_R->BWTR[0]&=~(0XF<<0);	//地址建立时间(ADDSET)清零 	 
		FMC_Bank1E_R->BWTR[0]&=~(0XF<<8);	//数据保存时间清零
		FMC_Bank1E_R->BWTR[0]|=5<<0;	 	//地址建立时间(ADDSET)为5个HCLK =25ns  	 
		FMC_Bank1E_R->BWTR[0]|=5<<8; 		//数据保存时间(DATAST)为5ns*4个HCLK=20ns
	} 	    
