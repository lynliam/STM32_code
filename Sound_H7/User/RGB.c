...
/*Some Static Colors------------------------------*/
const RGB_Color_TypeDef LED_RED      = {255,0,0};   //红色
const RGB_Color_TypeDef LED_GREEN    = {0,255,0};   //绿色
const RGB_Color_TypeDef LED_BLUE     = {0,0,255};   //深蓝色
const RGB_Color_TypeDef LED_SKY      = {0,255,255};  //天蓝色
const RGB_Color_TypeDef LED_MAGENTA  = {255,0,220};  //粉色
const RGB_Color_TypeDef LED_YELLOW   = {128,216,0};  //黄色
const RGB_Color_TypeDef LED_OEANGE   = {127,106,0};  //橘色
const RGB_Color_TypeDef LED_BLACK    = {0,0,0};    //无颜色
const RGB_Color_TypeDef LED_WHITE    = {255,255,255}; //白色
...
__section("._D1_Area") uint8_t buffer_mpu[1024] = {0};
uint32_t (*Pixel_Buf)[24] = buffer_mpu;       
...
{
	HAL_TIM_PWM_Start_DMA(&htim15, TIM_CHANNEL_1, (uint32_t *)Pixel_Buf,(Pixel_NUM+1)*24);
}
...
{
    RGB_SetMore_Color(0,63,LED_BLACK);  //清空所有的LED数据
